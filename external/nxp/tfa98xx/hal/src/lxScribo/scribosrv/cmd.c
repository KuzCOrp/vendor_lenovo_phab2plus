...
int tfa98xx_start(int profile, int *vstep, int channels); // runtime
int tfa98xx_stop(void);
void cmd_start(void *buf, int len) {
    int result=0;
    rprintf("%s\n",__FUNCTION__);
    ...
    if (sscanf(buf, "st %d %d %d", &profile, &vstep_l, &vstep_r) == 3) {
        vsteps[0] = vstep_l;
        vsteps[1] = vstep_r;
        result = tfa98xx_start(profile, vsteps, 2);
    }
    else
        result = 1;
...
void cmd_stop(void *buf, int len) {
    int result;
    rprintf("%s\n",__FUNCTION__);
            ...
            if (idx >= 2)
            {
                ...
                case CMD('o', 'd'): // 'do' ascii command
                  ...
                  //  hostWrite(activeSocket, cmd, len );
                    processed=0;
//                    char **args,*arg, *srv="server";//, "-ddummy", "--dump"};
//                    cmd[len]='\0'; //lf cr
//                    arg=strchr(cmd,'\r');
//                    if(arg) *arg='\0';
//                    // skip do until
//                    cmd+=3; //TODO search 1st non space
//                    if(len>3)
//                        argc=2;
//                    for(c=0;c<len;c++) {
//                        if (cmd[c] == ' ') {
