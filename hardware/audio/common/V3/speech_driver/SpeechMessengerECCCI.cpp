...
#if 1
extern void switch_narrow_wide_band(uint16_t mRfMode);
#endif
...
#if 1
				int mRfMode_value = 0;
				char mRfMode_temp[PROPERTY_VALUE_MAX];

				property_get("af.rf_mode", mRfMode_temp, "0");
				mRfMode_value = atoi(mRfMode_temp);

				if (mRfMode_value != mRfMode)
					switch_narrow_wide_band(mRfMode);//notify call to switch band mode between narrow band and wide band
#endif
//--Modified by shentaotato 2016.06.12
