...
char *audience_parse_xml_int[] = {
	"ES705 TX1 Enable", "Main Mic Power","Sub Mic Power", "Dolby Sync Count", "ESCORE Delay", "Internal Route", "Preset",
	"Preset1", "Preset2", "Preset3", "Preset4", "ES705 RX1 Enable", "ES705 RX2 Enable", "Dereverb Gain", "BWE High Band Gain",
	"SLIMbus Link Multi Channel", "Audio Custom Profile", "Voice Sense Training Model Length", "Voice Sense Detect Sensitivity",
	"Voice Activity Detect Sensitivity", "Continuous Voice Sense Preset", "Audio Focus Enable", "Enable/Disable Streaming PATH/Endpoint",
	"Get RDB data size", "ES705 Get Event Status", "CVQ Activate Keywords", "CVQ Sleep", "VS keyword length",
	"KW Overrun Error", "SDE Bearing Value", "SDE Salience Value", "SDE RMS Estimate Value"
};
unsigned int audience_parse_count = sizeof(audience_parse_xml_int) / sizeof(audience_parse_xml_int[0]);

//++ Modified for Voice Correction by shentaotao 2016.07.01
char *voice_parse_xml_int[] = {
	"Wide_Band_CT_Talk_Mic_Setting", "Narrow_Band_CT_Talk_Mic_Setting", "Wide_Band_FO_Talk_Mic_Setting", "Narrow_Band_FO_Talk_Mic_Setting",
	"Wide_Band_Three_Segment_HP_Talk_Mic_Setting", "Narrow_Band_Three_Segment_HP_Talk_Mic_Setting"
};
unsigned int voice_parse_count = sizeof(voice_parse_xml_int) / sizeof(voice_parse_xml_int[0]);
//-- Modified for Voice Correction by shentaotao 2016.07.01
...
status_t AudioALSADeviceConfigManager::ApplyDeviceTurnonSequenceByName(const char *DeviceName)
{
	...
	//++ Modified for Voice Correction by shentaotao 2016.07.01
	for (i = 0; i < voice_parse_count; i++)
	{
		if (!strcmp(voice_parse_xml_int[i], DeviceName))
		{
    ...
    if (descriptor->DeviceStatusCounter == 0)
    {
        for (count = 0; count < descriptor->mDeviceCltonVector.size(); count += 2)
        {
			...
			for (i = 0; i < audience_parse_count; i++)
			{
				if (!strcmp(audience_parse_xml_int[i], cltname.string()))
				{
					int value, ret;
					char *end;
					value = static_cast<int>(strtol(cltvalue.string(), &end, 10));
					ret = mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, cltname.string()), 0, value);
					//ALOGD("ApplyDeviceTurnonSequenceByName ret = %d value = %d\n", ret, value);
					break;
				}
			}

			if (audience_parse_count == i)
			{
        ...
        for (count = 0; count < descriptor->mDeviceCltoffVector.size(); count += 2)
        {
			...
			for (i = 0; i < audience_parse_count; i++)
			{
				if (!strcmp(audience_parse_xml_int[i], cltname.string()))
				{
					...
					value = static_cast<int>(strtol(cltvalue.string(), &end, 10));
					ret = mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, cltname.string()), 0, value);
					ALOGD("ApplyDeviceTurnoffSequenceByName ret = %d value = %d\n", ret, value);
					break;
				}
			}
			...
			if (audience_parse_count == i)
			{
				if (mixer_ctl_set_enum_by_string(mixer_get_ctl_by_name(mMixer, cltname.string()), cltvalue.string()))
				{
					ALOGE("Error: ApplyDeviceTurnoffSequence  cltname.string () = %s cltvalue.string () = %s", cltname.string(), cltvalue.string());
					ASSERT(false);
				}
			}
        }
