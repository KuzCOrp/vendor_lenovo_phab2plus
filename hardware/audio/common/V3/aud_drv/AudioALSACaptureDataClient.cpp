...
//++Modified by shentaotato 2016.06.12 to close MIC algorithm
	//close MIC algorithm except Headset && Bluetooth MIC
	if ((mStreamAttributeTarget->input_device == AUDIO_DEVICE_IN_WIRED_HEADSET) ||
		(mStreamAttributeTarget->input_device == AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET))
	{
		ALOGD("STT:enter if %s, 0x%x", __FUNCTION__, mStreamAttributeTarget->input_device);
		mBypassBesRecord = false;
	}
	else
	{
		ALOGD("STT:enter else %s, 0x%x", __FUNCTION__, mStreamAttributeTarget->input_device);
