...
//Added by Shentaotao start
/* ++Audience CVQ Implementation++ */
#include <cutils/properties.h>
#include "CVQStream.h"

#define AUDIENCE_CVQ_PROP    ("persist.sys.audience.ustrhal")
/* --Audience CVQ Implementation-- */
//Added by Shentaotao end
...
static audio_channel_mask_t in_get_channels(const struct audio_stream *stream)
    ...
    */
                           ...
                           size_t bytes)
    {
		...
		/* ++Audience CVQ Implementation++ */
		int ret = -1;
		if (in->stream.is_cvq == 1) {
			struct CVQStream* cvqstreamptr;
			cvqstreamptr = (struct CVQStream*)stream->mcvqstream;

			ALOGV("Before cvq readdirect");
			ret = cvqstreamptr->readdirect(cvqstreamptr, buffer, bytes);
			ALOGV("After cvq readdirect byte = %d, ret = %d", bytes, ret);
			ret = 0;
		} else {
		/* --Audience CVQ Implementation-- */
		//Added by Shentaotao end
			ret = in->legacy_in->read(buffer, bytes);
		//Added by Shentaotao start
		/* ++Audience CVQ Implementation++ */
		} // if (in->stream.is_cvq == 1) { ... } else { ...
		/* --Audience CVQ Implementation-- */
		//Added by Shentaotao end
		return ret;
    }
...
err_open:
                                      ...
                                      audio_source_t source /*__unused*/)
    {
		...
		//Added by Shentaotao start
		/* ++Audience CVQ Implementation++ */
		char aud_ustrhal[128];
		property_get(AUDIENCE_CVQ_PROP, aud_ustrhal, "0");
		ALOGD("adev_open_input_stream %s", aud_ustrhal);
		if (!strncmp(aud_ustrhal, "1", 2)) {
				struct CVQStream *cvqstreamptr;
			ALOGD("audience cvq enabled");
			in->stream.is_cvq = 1;
			cvqstreamptr = (struct CVQStream*)malloc(sizeof(struct CVQStream));
			if(cvqstreamptr == NULL) {
				ALOGE("Error in assigning mcvqstream");
				goto err_cvq;
			}
			in->stream.mcvqstream = cvqstreamptr;
			cvq_init(cvqstreamptr);
				cvqstreamptr->open(cvqstreamptr);
		}
		/* --Audience CVQ Implementation-- */
		//Added by Shentaotao end
       ...
       //devices = convert_audio_device(devices, HAL_API_REV_2_0, HAL_API_REV_1_0);
		...
		//ALOGD("STT: adev_open_input_stream flags=%d\n", flags);
...
err_open:
                                        ...
                                        struct audio_stream_in *stream)
    {
		...
		if(in->stream.is_cvq == 1) {
			struct CVQStream* cvqstreamptr;
			cvqstreamptr = (struct CVQStream*)stream->mcvqstream;
			cvqstreamptr->close(cvqstreamptr);
			free(in->stream.mcvqstream);
			free(stream);
			ALOGD("#### After CVQ close ####");
		} else {
		/* --Audience CVQ Implementation-- */
		//Added by Shentaotao end
