...
#define TS3A227EConnectorType_PATH "/sys/bus/platform/drivers/Accdet_Driver/TS3A227EConnectorType"
#define TS3A227EConnectorType_Notify_PATH "/sys/bus/platform/drivers/Accdet_Driver/TS3A227EConnectorType_Notify"
int read_TS3A227EConnectorType(void)
{
	char buf[10];
	int len, fd, ret = 0;

	fd = open(TS3A227EConnectorType_PATH, O_RDONLY);
	if (fd < 0) {
		ALOGE("%s(), Error opening %s\n", __FUNCTION__, TS3A227EConnectorType_PATH);
	}

	len = read(fd, buf, 10);
	if (len < 0) {
		ALOGE("%s(), Error reading %s\n", __FUNCTION__, TS3A227EConnectorType_PATH);
	}
	close(fd);
...
void write_TS3A227EConnectorType_Notify(int flag)
{
	char value[10];
	int len, fd, ret;

	fd = open(TS3A227EConnectorType_Notify_PATH, O_WRONLY);
	if (fd < 0) {
		ALOGE("%s(), Error opening %s\n", __FUNCTION__, TS3A227EConnectorType_Notify_PATH);
	}
...
status_t AudioALSASpeechPhoneCallController::open(const audio_mode_t audio_mode, const audio_devices_t output_devices, const audio_devices_t input_device)
{
    ...
    audio_devices_t input_device_l;
	...
	//++Added by shentaotao 2016.06.07
	if(mAudioMode == AUDIO_MODE_IN_CALL)
	{
		switch (output_devices)
		{
			case AUDIO_DEVICE_OUT_SPEAKER:
			{
				int Type_number = read_TS3A227EConnectorType();
				ALOGD("STT: %s(), Type_number = %d\n", __FUNCTION__, Type_number);

				if (Type_number == 2 || Type_number == 3)
					write_TS3A227EConnectorType_Notify(1);
				else
					write_TS3A227EConnectorType_Notify(0);
				break;
			}
			case AUDIO_DEVICE_OUT_WIRED_HEADPHONE:
			case AUDIO_DEVICE_OUT_EARPIECE:
    ...
    else
    {
		...
		//ALOGD("JIAXIN TRACE 1--SPEECH OPEN!");
		if(audio_mode == AUDIO_MODE_IN_CALL)
		{
			switch (output_devices)
			{
				case AUDIO_DEVICE_OUT_WIRED_HEADPHONE:
				{
					input_device_l = AUDIO_DEVICE_IN_USAGE_1;
					break;
				}
				case AUDIO_DEVICE_OUT_EARPIECE:
				{
					input_device_l = AUDIO_DEVICE_IN_USAGE_2;
					break;
				}
				case AUDIO_DEVICE_OUT_SPEAKER:
				{
					input_device_l = AUDIO_DEVICE_IN_USAGE_3;
					break;
				}
				default:
				{
					ALOGW("%s(), no support such output_devices(0x%x), default use AUDIO_DEVICE_IN_BUILTIN_MIC(0x%x) as input_device",
						__FUNCTION__, output_devices, AUDIO_DEVICE_IN_BUILTIN_MIC);
					if (AUDIO_DEVICE_IN_WIRED_HEADSET == input_device)
					{
						ALOGD("STT: %s Use 4 Segment HeadSet\n", __FUNCTION__);
						input_device_l = input_device;
					}
					else
						input_device_l = AUDIO_DEVICE_IN_BUILTIN_MIC;
					break;
				}
			}
			mHardwareResourceManager->startInputDevice(input_device_l);
		}
		else
		{
...
status_t AudioALSASpeechPhoneCallController::routing(const audio_devices_t new_output_devices, const audio_devices_t new_input_device)
{
    ...
    {
		//ALOGD("JIAXIN ADD ROUTING ++++");
		if(mAudioMode == AUDIO_MODE_IN_CALL)
		{
			audio_devices_t input_device_t = new_input_device;
			switch (new_output_devices)
			{
				case AUDIO_DEVICE_OUT_WIRED_HEADPHONE:
				{
					input_device_t = AUDIO_DEVICE_IN_USAGE_1;
					break;
				}
				case AUDIO_DEVICE_OUT_EARPIECE:
				{
					input_device_t = AUDIO_DEVICE_IN_USAGE_2;
					break;
				}
				case AUDIO_DEVICE_OUT_SPEAKER:
				{
					input_device_t = AUDIO_DEVICE_IN_USAGE_3;
					break;
				}
				default:
				{
					 ALOGW("%s(), no support such output_devices(0x%x), default use AUDIO_DEVICE_IN_BUILTIN_MIC(0x%x) as input_device",
						__FUNCTION__, new_output_devices, AUDIO_DEVICE_IN_BUILTIN_MIC);
					if (AUDIO_DEVICE_IN_WIRED_HEADSET == input_device)
					{
						ALOGD("STT: %s Use 4 Segment HeadSet\n", __FUNCTION__);
						...
						input_device_t = AUDIO_DEVICE_IN_BUILTIN_MIC;
					break;
				}
			}

			// Set PMIC digital/analog part - uplink has pop, open first
			mHardwareResourceManager->startInputDevice(input_device_t);
		}
		else
		{
			mHardwareResourceManager->startInputDevice(input_device);
