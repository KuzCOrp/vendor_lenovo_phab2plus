...
//++Modified by shentaotato 2016.06.12 for VOIP
        ...
        if (mStreamAttributeTarget.BesRecord_Info.besrecord_voip_enable == true)
        {
            ...
            {
				inputdevice = AUDIO_DEVICE_IN_VOIP_SPEAKER;
#if 0
                if (inputdevice == AUDIO_DEVICE_IN_BUILTIN_MIC)
                {
                    if (USE_REFMIC_IN_LOUDSPK == 1)
            ...
            {
				inputdevice = AUDIO_DEVICE_IN_VOIP_EARPIECE;
#if 0
			...
			else if (output_devices == AUDIO_DEVICE_OUT_WIRED_HEADPHONE)   //receiver -> headphone
			{
				inputdevice = AUDIO_DEVICE_IN_VOIP_HEADPHONE;
			}
        }

		ALOGV("STT: %s(), input_device: 0x%x, 0x%x", __FUNCTION__, mStreamAttributeTarget.input_device, inputdevice);
