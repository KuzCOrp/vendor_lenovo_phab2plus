...
//++Modified by shentaotato 2016.06.12 for wide narrow band
#define NAME_LENGTH 60
audio_devices_t new_device_for_narrow_wide_band = AUDIO_DEVICE_IN_USAGE_2;
char turn_on_off_name[NAME_LENGTH] = {0};
void switch_narrow_wide_band(uint16_t rfMode)
{
	ALOGD("STT: %s new_device_for_narrow_wide_band = 0x%x, rfMode = %d, turn_on_off_name = %s\n",
		__FUNCTION__, new_device_for_narrow_wide_band, rfMode, turn_on_off_name);

	AudioALSADeviceConfigManager::getInstance()->ApplyDeviceTurnoffSequenceByName(turn_on_off_name);
	memset(turn_on_off_name, 0, NAME_LENGTH);
	switch (new_device_for_narrow_wide_band)
	{
		case AUDIO_DEVICE_IN_USAGE_1:
		{
			if (rfMode & 0x8)
			{
				AudioALSADeviceConfigManager::getInstance()->ApplyDeviceTurnonSequenceByName("Wide_Band_Three_Segment_HP_Talk_Mic_Setting");
				memcpy(turn_on_off_name, "Wide_Band_Three_Segment_HP_Talk_Mic_Setting", NAME_LENGTH);
			}
			else
			{
				AudioALSADeviceConfigManager::getInstance()->ApplyDeviceTurnonSequenceByName("Narrow_Band_Three_Segment_HP_Talk_Mic_Setting");
				memcpy(turn_on_off_name, "Narrow_Band_Three_Segment_HP_Talk_Mic_Setting", NAME_LENGTH);
			}
			break;
		}
		case AUDIO_DEVICE_IN_USAGE_2:
		{
			if (rfMode & 0x8)
			{
				AudioALSADeviceConfigManager::getInstance()->ApplyDeviceTurnonSequenceByName("Wide_Band_CT_Talk_Mic_Setting");
				memcpy(turn_on_off_name, "Wide_Band_CT_Talk_Mic_Setting", NAME_LENGTH);
			}
			else
			{
				AudioALSADeviceConfigManager::getInstance()->ApplyDeviceTurnonSequenceByName("Narrow_Band_CT_Talk_Mic_Setting");
				memcpy(turn_on_off_name, "Narrow_Band_CT_Talk_Mic_Setting", NAME_LENGTH);
			}
			break;
		}
		case AUDIO_DEVICE_IN_USAGE_3:
		{
			if (rfMode & 0x8)
			{
				AudioALSADeviceConfigManager::getInstance()->ApplyDeviceTurnonSequenceByName("Wide_Band_FO_Talk_Mic_Setting");
				memcpy(turn_on_off_name, "Wide_Band_FO_Talk_Mic_Setting", NAME_LENGTH);
			}
			else
			{
				AudioALSADeviceConfigManager::getInstance()->ApplyDeviceTurnonSequenceByName("Narrow_Band_FO_Talk_Mic_Setting");
				memcpy(turn_on_off_name, "Narrow_Band_FO_Talk_Mic_Setting", NAME_LENGTH);
			}
			break;
		}
		default:
		{
			ALOGD("STT: %s unkown AUDIO_DEVICE", __FUNCTION__);
			break;
		}
	}
}

static int mRfMode = 0, vq_trainging_mic_value = 0, mic_loopback_value = 0, mic_sealing_number_value = 0;
status_t AudioALSAHardwareResourceManager::startInputDevice(const audio_devices_t new_device, audio_input_flags_t flags)
{
    ALOGD("+%s(), new_device = 0x%x, mInputDevice = 0x%x, mStartInputDeviceCount = %d, mMicInverse=%d, flags=%d", __FUNCTION__, new_device, mInputDevice, mStartInputDeviceCount, mMicInverse, flags);

	AudioAutoTimeoutLock _l(mLockin);

	char property_rfmode[PROPERTY_VALUE_MAX];
	char vq_trainging_mic[PROPERTY_VALUE_MAX] = {0}, call_mic_number[PROPERTY_VALUE_MAX] = {0}, mic_loopback_number[PROPERTY_VALUE_MAX] = {0};

	property_get("af.rf_mode", property_rfmode, "0");
	mRfMode = atoi(property_rfmode);

	property_get("persist.sys.vq.training", vq_trainging_mic, "0");
	vq_trainging_mic_value = atoi(vq_trainging_mic);

	property_get("persist.sys.mic", call_mic_number, "0");
	mic_sealing_number_value = atoi(call_mic_number);

	property_get("persist.sys.mic.loopback", mic_loopback_number, "0");
	mic_loopback_value = atoi(mic_loopback_number);
	ALOGD("STT: startInputDevice mRfMode = %d, persist.sys.mic.loopback=%d, persist.sys.mic=%d, persist.sys.vq.training=%d\n",
		mRfMode, mic_loopback_value, mic_sealing_number_value, vq_trainging_mic_value);
	...
	new_device_for_narrow_wide_band = new_device;
	memset(turn_on_off_name, 0, NAME_LENGTH);
	...
	switch (new_device)
	{
		case AUDIO_DEVICE_IN_USAGE_1:
		{
			if (mRfMode & 0x8)
			{
				mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("Wide_Band_Three_Segment_HP_Talk_Mic_Setting");
				memcpy(turn_on_off_name, "Wide_Band_Three_Segment_HP_Talk_Mic_Setting", NAME_LENGTH);
			}
			else
			{
				mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("Narrow_Band_Three_Segment_HP_Talk_Mic_Setting");
				memcpy(turn_on_off_name, "Narrow_Band_Three_Segment_HP_Talk_Mic_Setting", NAME_LENGTH);
			}
			break;
		}
		case AUDIO_DEVICE_IN_USAGE_2:
		{
			if (mic_sealing_number_value == 1)
			{
				mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("CALL_ES814_MIC_1");
			} else if (mic_sealing_number_value == 2)
			{
				mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("CALL_ES814_MIC_2");
			} else if (mic_sealing_number_value == 3)
			{
				mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("CALL_ES814_MIC_3");
			} else {
				if (mRfMode & 0x8)
				{
					mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("Wide_Band_CT_Talk_Mic_Setting");
					memcpy(turn_on_off_name, "Wide_Band_CT_Talk_Mic_Setting", NAME_LENGTH);
				}
				else
				{
					mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("Narrow_Band_CT_Talk_Mic_Setting");
					memcpy(turn_on_off_name, "Narrow_Band_CT_Talk_Mic_Setting", NAME_LENGTH);
				}
			}
			break;
		...
		case AUDIO_DEVICE_IN_USAGE_3:
		{
			if (mRfMode & 0x8)
			{
				mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("Wide_Band_FO_Talk_Mic_Setting");
				memcpy(turn_on_off_name, "Wide_Band_FO_Talk_Mic_Setting", NAME_LENGTH);
			}
			else
			{
				mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("Narrow_Band_FO_Talk_Mic_Setting");
				memcpy(turn_on_off_name, "Narrow_Band_FO_Talk_Mic_Setting", NAME_LENGTH);
			}
			break;
		}
		case AUDIO_DEVICE_IN_VOIP_HEADPHONE:
		{
			mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("VOIP_HEADPHONE_Setting");
			break;
		}
		case AUDIO_DEVICE_IN_VOIP_EARPIECE:
		{
			mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("VOIP_EARPIECE_Setting");
			break;
		}
		case AUDIO_DEVICE_IN_VOIP_SPEAKER:
		{
			mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("VOIP_SPEAKER_Setting");
			break;
		}
		default:
			break;
	...
	if (flags)//used for FactoryTest Mic 1,2,3 && REDUCTION_RECORDING && DIRECTION_RECORDING
	{
		switch (flags)
		{
			case 1:
			...
			case AUDIO_INPUT_FLAG_TEST1:
			{
				mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("FactoryTest_ES814_MIC_1");
				break;
			}
			case AUDIO_INPUT_FLAG_TEST2:
			{
				mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("FactoryTest_ES814_MIC_2");
				break;
			}
			case AUDIO_INPUT_FLAG_TEST3:
			{
				mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("FactoryTest_ES814_MIC_3");
				break;
			}
			case AUDIO_INPUT_FLAG_TEST4:
			{
				if (new_device == AUDIO_DEVICE_IN_WIRED_HEADSET)
				{
					setMIC1Mode(true);
					setMIC2Mode(true);
					mDeviceConfigManager->ApplyDeviceTurnonSequenceByName(AUDIO_DEVICE_HEADSET_MIC);
					...
					mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("REDUCTION_RECORDING");
				break;
			}
			case AUDIO_INPUT_FLAG_TEST5:
			{
				if (new_device == AUDIO_DEVICE_IN_WIRED_HEADSET)
				{
					setMIC1Mode(true);
					...
					mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("DIRECTION_RECORDING");
				break;
			}
			case AUDIO_INPUT_FLAG_TEST6:
			{
				//Reserved
				break;
			}
			case AUDIO_INPUT_FLAG_TEST10://NOT USE
			{
				mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("VQ_TRAINING");
				break;
			}
			default:
				break;
	...
	else if (flags == 0)
	{
		if (new_device == AUDIO_DEVICE_IN_BUILTIN_MIC)
		{
			...
			if (mBuiltInMicSpecificType == BUILTIN_MIC_MIC1_ONLY)
			{
				if (mMicInverse == true)
				{
					if (mic_loopback_value == 1)
						mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("Loopback_ES814_MIC_1");
					else
			...
			else if (mBuiltInMicSpecificType == BUILTIN_MIC_MIC2_ONLY)
			{
				...
				{
					if (mic_loopback_value == 1)
						mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("Loopback_ES814_MIC_2");
					else
			...
			else if (mBuiltInMicSpecificType == BUILTIN_MIC_MIC3_ONLY)
			{
				...
				{
					if (mic_loopback_value == 1)
						mDeviceConfigManager->ApplyDeviceTurnonSequenceByName("Loopback_ES814_MIC_3");
					else
						mDeviceConfigManager->ApplyDeviceTurnonSequenceByName(AUDIO_DEVICE_BUILTIN_MIC_MIC3_INVERSE);
				}
			}
...
status_t AudioALSAHardwareResourceManager::stopInputDevice(const audio_devices_t stop_device, audio_input_flags_t flags)
{
    ALOGD("+%s(), mInputDevice = 0x%x, stop_device = 0x%x, mStartInputDeviceCount = %d, mMicInverse=%d, flags=%d", __FUNCTION__, mInputDevice, stop_device, mStartInputDeviceCount, mMicInverse, flags);
	...
	switch (stop_device)
	{
		case AUDIO_DEVICE_IN_USAGE_1:
		{
			mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName(turn_on_off_name);
			break;
		}
		case AUDIO_DEVICE_IN_USAGE_2:
		{
			if (mic_sealing_number_value == 1)
			{
				mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("CALL_ES814_MIC_1");
			} else if (mic_sealing_number_value == 2)
			{
				mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("CALL_ES814_MIC_2");
			} else if (mic_sealing_number_value == 3)
			{
				mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("CALL_ES814_MIC_3");
			} else {
				mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName(turn_on_off_name);
			}
			break;
		...
		case AUDIO_DEVICE_IN_VOIP_HEADPHONE:
		{
			mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("VOIP_HEADPHONE_Setting");
			break;
		}
		case AUDIO_DEVICE_IN_VOIP_EARPIECE:
		{
			mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("VOIP_EARPIECE_Setting");
			break;
		}
		case AUDIO_DEVICE_IN_VOIP_SPEAKER:
		{
			mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("VOIP_SPEAKER_Setting");
			break;
		}
		default:
			break;
	...
	if (flags)//used for FactoryTest Mic 1,2,3 && REDUCTION_RECORDING && DIRECTION_RECORDING
	{
		switch (flags)
		{
			...
			case AUDIO_INPUT_FLAG_TEST1:
			{
				mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("FactoryTest_ES814_MIC_1");
				break;
			}
			case AUDIO_INPUT_FLAG_TEST2:
			{
				mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("FactoryTest_ES814_MIC_2");
				break;
			}
			case AUDIO_INPUT_FLAG_TEST3:
			{
				mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("FactoryTest_ES814_MIC_3");
				break;
			}
			case AUDIO_INPUT_FLAG_TEST4:
			{
				if (stop_device == AUDIO_DEVICE_IN_WIRED_HEADSET)
				{
					setMIC1Mode(true);
					mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName(AUDIO_DEVICE_HEADSET_MIC);
				}
				else
					mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("REDUCTION_RECORDING");
				break;
			}
			case AUDIO_INPUT_FLAG_TEST5:
			{
				if (stop_device == AUDIO_DEVICE_IN_WIRED_HEADSET)
				{
					...
					mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("DIRECTION_RECORDING");
				break;
			}
			case AUDIO_INPUT_FLAG_TEST6:
			{
			...
			case AUDIO_INPUT_FLAG_TEST10://NOT USE
			{
				mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("VQ_TRAINING");
				break;
			}
			default:
				break;
	...
	else if (flags == 0)
	{
		if (stop_device == AUDIO_DEVICE_IN_BUILTIN_MIC)
		{
			if (mBuiltInMicSpecificType == BUILTIN_MIC_MIC1_ONLY)
			{
				...
				{
					if (mic_loopback_value == 1)
						mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("Loopback_ES814_MIC_1");
					else
			...
			if (mBuiltInMicSpecificType == BUILTIN_MIC_MIC2_ONLY)
			{
				...
				{
					if (mic_loopback_value == 1)
						mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("Loopback_ES814_MIC_2");
					else
			...
			else if (mBuiltInMicSpecificType == BUILTIN_MIC_MIC3_ONLY)
			{
				...
				{
					if (mic_loopback_value == 1)
						mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName("Loopback_ES814_MIC_3");
					else
						mDeviceConfigManager->ApplyDeviceTurnoffSequenceByName(AUDIO_DEVICE_BUILTIN_MIC_MIC3_INVERSE);
				}
			}
			else if (mBuiltInMicSpecificType == BUILTIN_MIC_DEFAULT)
			{
...
//--Modified by shentaotato 2016.06.12
