...
static const uint32_t             kDefaultOutputSourceSampleRate  = 48000;
...
#if 1 //++ Added by shentaotao for dolby verify
	if (mStreamAttributeSource.output_devices == AUDIO_DEVICE_OUT_SPEAKER)
	{
		//ALOGD("%s(), DoStereoMonoConvert start", __FUNCTION__);
		int32_t len = sizeof(int32_t) * 2;
		int32_t FinalValue = 0;
		int32_t *Sample = (int32_t *)buffer;
		size_t bytes_t = bytes;
		...
		//ALOGD("%s(), DoStereoMonoConvert end", __FUNCTION__);
	}
#endif //-- Added by shentaotao for dolby verify
    size_t outputSize = 0;
    if (mSuspendCount > 0 ||
