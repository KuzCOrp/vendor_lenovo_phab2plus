...
status_t AudioALSAStreamManager::routingOutputDevice(AudioALSAStreamOut *pAudioALSAStreamOut, const audio_devices_t current_output_devices, audio_devices_t output_devices)
{
    ...
    else
    {
        if (mStreamOutVector.size() > 0 || mStreamInVector.size() > 0)
        {
            ...
            if (mFMController->getFmEnable() == true)
            {
                if(android_audio_legacy::AudioSystem::popCount(current_output_devices)!= android_audio_legacy::AudioSystem::popCount(output_devices))
                    mFMController->routing(current_output_devices, output_devices);//switch between SPK+HP and HP (ringtone)
                else
